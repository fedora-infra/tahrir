...
def json_rarities(rare=None):
    ...
    if rare:
        rarity_objc = (
            g.tahrirdb.session.query(m.Rarity)
            .filter(m.Rarity.name != "O")
            .filter_by(name=rare.upper())
            .first()
        )
        if not rarity_objc:
        ...
        rarity_full = g.tahrirdb.session.query(m.Rarity).filter(m.Rarity.name != "O").all()
        return jsonify(
