...
import { useCreationAvermentMutation, useSearchAccoladeQuery, useSearchIdentityQuery } from "../../features/call.js";
...
export default function AssertionCreationForm() {
  ...
  const [accoladeSearch, makeAccoladeSearch] = useState("");
  ...
  const { data: accoladeResult } = useSearchAccoladeQuery(accoladeSearch, {
    skip: accoladeSearch.length < 4,
  });
  const { data: identityResult } = useSearchIdentityQuery(identitySearch, {
  ...
  const handleAccoladeSelect = (accolade) => {
    makeAccoladeSearch(accolade.name);
            ...
            <div className="position-relative">
              <FloatingLabel controlId="assertCreateBadge" label="Badge*">
                <Form.Control
                  ...
                  onChange={(e) => {
                    makeAccoladeSearch(e.target.value);
                    makeAccoladeDropdownShow(e.target.value.length >= 4);
                  }}
                  onFocus={() => accoladeSearch.length >= 4 && makeAccoladeDropdownShow(true)}
                  onBlur={() => setTimeout(() => makeAccoladeDropdownShow(false), 150)}
                  required
                />
              </FloatingLabel>
              {accoladeSearch.length >= 4 &&
                accoladeResult &&
